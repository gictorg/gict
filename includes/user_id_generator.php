...
function generateUniqueUserId($user_type)
{
    try {
        ...
        $type_mapping = [
            'faculty' => ['prefix' => 'FA', 'type_id' => 3],
            'student' => ['prefix' => 'SA', 'type_id' => 2],
            'admin' => ['prefix' => 'AA', 'type_id' => 1]
        ];
...
{
    // Pattern: [FA/SA/AA][4-digit-year][3-digit-number]
    $pattern = '/^(FA|SA|AA)\d{7}$/';
    return preg_match($pattern, strtoupper($user_id)) === 1;
}
...
function parseUserId($user_id)
{
    ...
    $type_prefix = substr($user_id, 0, 2);
    $year = substr($user_id, 2, 4);
    $number = substr($user_id, 6, 3);
