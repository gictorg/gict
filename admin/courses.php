...
if (isset($_GET['edit_sub']) && is_numeric($_GET['edit_sub'])) {
    ...
    try {
        $sql = "SELECT sc.* FROM sub_courses sc 
                JOIN courses c ON sc.course_id = c.id 
                WHERE sc.id = ?";
        $edit_sub_course = getRow($sql, [$sub_course_id]);
                                        ...
                                        <td>
                                            <div class="action-buttons">
                                                <button class="btn-small btn-edit"
                                                    onclick="editCourse(<?php echo $course['id']; ?>)">
                                                    <i class="fas fa-edit"></i> Edit
                                                </button>
                                                <button class="btn-small btn-toggle"
                                                    onclick="toggleCourseStatus(<?php echo $course['id']; ?>, '<?php echo $course['status'] === 'active' ? 'inactive' : 'active'; ?>')">
                            ...
                            <tbody>
                                <?php foreach ($sub_courses as $sub_course): ?>
                                    <tr>
                                        ...
                                        <td class="course-description"
                                            title="<?php echo htmlspecialchars($sub_course['description']); ?>">
                                        ...
                                        <td>
                                            <div class="action-buttons">
                                                ...
                                                <button class="btn-small btn-toggle"
                                                    onclick="toggleSubCourseStatus(<?php echo $sub_course['id']; ?>, '<?php echo $sub_course['status'] === 'active' ? 'inactive' : 'active'; ?>')">
                                                    <i class="fas fa-toggle-on"></i>
                                                    <?php echo $sub_course['status'] === 'active' ? 'Deactivate' : 'Activate'; ?>
                                                </button>
                                                <button class="btn-small btn-delete"
                                                    onclick="deleteSubCourse(<?php echo $sub_course['id']; ?>, '<?php echo htmlspecialchars($sub_course['name']); ?>')">
                                                    <i class="fas fa-trash"></i> Delete
                                                </button>
                                            </div>
                                        </td>
            ...
            <form method="POST" action="courses.php" enctype="multipart/form-data">
                <div class="modal-body">
                    <input type="hidden" name="action"
                        value="<?php echo isset($edit_course) ? 'update_course' : 'add_course'; ?>">
                    ...
                    <div class="form-group full-width">
                        ...
                        <small class="form-text">Upload JPG, PNG, or GIF image (max 500KB). Image will be uploaded to
                            Cloudinary.</small>
    ...
    <div id="subCourseModal" class="modal">
        <div class="modal-content">
            ...
            <form method="POST" action="courses.php">
                <div class="modal-body">
                    <input type="hidden" name="action"
                        value="<?php echo isset($edit_sub_course) ? 'update_sub_course' : 'add_sub_course'; ?>">
                    ...
                    <div class="form-row">
                        ...
                        <div class="form-group">
                            ...
                            <select id="course_id" name="course_id" required>
                                ...
                                <?php foreach ($courses as $course): ?>
                                    <option value="<?php echo $course['id']; ?>" <?php echo (isset($edit_sub_course) && $edit_sub_course['course_id'] == $course['id']) ? 'selected' : ''; ?>>
