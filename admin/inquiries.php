...
<head>
    ...
    <style>
        ...
        .page-header h1 i { color: #667eea; font-size: 32px; }
        .stats-grid {
            display: grid;
        ...
        .stat-card h3 { margin: 0 0 8px 0; font-size: 32px; font-weight: 800; color: #333; }
        .stat-card p { margin: 0; color: #6c757d; font-weight: 600; letter-spacing: .3px; text-transform: uppercase; font-size: 12px; }
        .table-responsive { overflow-x: auto; background: white; border-radius: 12px; box-shadow: 0 4px 15px rgba(0, 0, 0, 0.08); border: 1px solid #e9ecef; }
        .table { width: 100%; border-collapse: collapse; background: white; }
        .table thead { background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; }
        .table th { padding: 16px 12px; text-align: left; font-weight: 600; font-size: 14px; text-transform: uppercase; letter-spacing: 0.5px; border: none; white-space: nowrap; }
        .table td { padding: 16px 12px; vertical-align: middle; color: #374151; font-size: 14px; border-bottom: 1px solid #f1f5f9; }
        .table tbody tr:hover { background-color: #f8fafc; }
        .status-badge { display: inline-block; padding: 6px 12px; border-radius: 20px; font-size: 12px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; }
        .status-new { background: #fef3c7; color: #92400e; }
        .status-contacted { background: #dbeafe; color: #1e40af; }
        .status-enrolled { background: #d1fae5; color: #065f46; }
        .status-closed { background: #fee2e2; color: #991b1b; }
        .action-buttons { display: flex; gap: 8px; flex-wrap: wrap; }
        .btn { padding: 8px 12px; border: none; border-radius: 6px; cursor: pointer; font-size: 12px; font-weight: 600; display: inline-flex; align-items: center; gap: 6px; }
        ...
        .modal-content { background-color: white; margin: 5% auto; padding: 30px; border-radius: 12px; width: 90%; max-width: 600px; max-height: 80vh; overflow-y: auto; }
        ...
        .form-group select { width: 100%; padding: 0.75rem; border: 2px solid #e9ecef; border-radius: 6px; font-size: 1rem; }
        @media (max-width: 768px) { .action-buttons { flex-direction: column; } .table { font-size: 12px; } .table th, .table td { padding: 12px 8px; } }
    </style>
</head>
    ...
    <div class="admin-layout">
        <aside class="admin-sidebar">
            ...
            <ul class="sidebar-nav">
                ...
                <li><a class="active" href="inquiries.php"><i class="fas fa-question-circle"></i> Course Inquiries</a></li>
        ...
        <header class="admin-topbar">
            <div class="topbar-left">
                ...
                <div class="breadcrumbs">
                    ...
                    <a href="../dashboard.php">Dashboard</a> / Inquiries
                </div>
            </div>
        </header>
        ...
        <main class="admin-content">
            ...
            <div class="stats-grid">
                <div class="stat-card"><h3><?php echo $stats['new_count']; ?></h3><p>New Inquiries</p></div>
                <div class="stat-card"><h3><?php echo $stats['contacted_count']; ?></h3><p>Contacted</p></div>
                <div class="stat-card"><h3><?php echo $stats['enrolled_count']; ?></h3><p>Enrolled</p></div>
                <div class="stat-card"><h3><?php echo $stats['closed_count']; ?></h3><p>Closed</p></div>
            </div>
                        ...
                        <?php else: ?>
                            <?php foreach ($inquiries as $inquiry): ?>
                                <tr>
                                    ...
                                    <td>
                                        <?php if ($inquiry['sub_course_name']): ?>
                                            ...
                                            <?php if ($inquiry['course_name']): ?>
                                                <br><small class="text-muted"><?php echo htmlspecialchars($inquiry['course_name']); ?></small>
                                            <?php endif; ?>
                                        <?php else: ?>
                                    ...
                                    <td>
                                        <span class="status-badge status-<?php echo $inquiry['status']; ?>"><?php echo ucfirst($inquiry['status']); ?></span>
                                    </td>
                                    <td><?php echo date('M d, Y H:i', strtotime($inquiry['created_at'])); ?></td>
                                    <td>
                                        <div class="action-buttons">
                                            <button class="btn btn-primary" onclick="viewInquiry(<?php echo $inquiry['id']; ?>)"><i class="fas fa-eye"></i> View</button>
                                            <button class="btn btn-warning" onclick="updateStatus(<?php echo $inquiry['id']; ?>, '<?php echo $inquiry['status']; ?>')"><i class="fas fa-edit"></i> Status</button>
                                            <button class="btn btn-danger" onclick="deleteInquiry(<?php echo $inquiry['id']; ?>)"><i class="fas fa-trash"></i> Delete</button>
                                        </div>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
